...
def draw_edgelets(image, edgelets, lineThickness = 2):
    locations, directions, strengths = edgelets
    for i in range(locations.shape[0]):
        xax = [locations[i, 0] - directions[i, 0] * strengths[i] / 2,
               locations[i, 0] + directions[i, 0] * strengths[i] / 2]
        ...
        xax = np.int32(xax)
        yax = np.int32(yax)
        cv2.line(image, (xax[0], yax[0]), (xax[1], yax[1]), (0,255,0), lineThickness)       
...
def detect_feature_harris(src_gray, blockSize = 3, apertureSize = 3, qualityLevel = 50, max_qualityLevel = 100, maxCorners= 1000):
    # My Harris matrix -- Using cornerEigenValsAndVecs
    myHarris_dst = cv2.cornerEigenValsAndVecs(src_gray, blockSize, apertureSize)
    ...
    h = Mc.shape[0]
    w = Mc.shape[1]
#    print('print(np.shape(myShiTomasi_dst)))')
#    print(np.shape(myShiTomasi_dst))
    Mc = np.reshape(Mc, (h * w))
    Mc[Mc.argsort()[0:-maxCorners+1]] = myHarris_minVal
    Mc = np.reshape(Mc, (h, w))
    
    feature_points = []
    for i in range(src_gray.shape[0]):
        for j in range(src_gray.shape[1]):
            if Mc[i,j] > myHarris_minVal + ( myHarris_maxVal - myHarris_minVal )*qualityLevel/max_qualityLevel:
               feature_points.append([[i, j]])
    return feature_points

def detect_feature_ShiTomasi(src_gray, blockSize = 3, apertureSize = 3, qualityLevel = 50, max_qualityLevel = 100, maxCorners= 1000):
    myShiTomasi_dst = cv2.cornerMinEigenVal(src_gray, blockSize, apertureSize)
    myShiTomasi_minVal, myShiTomasi_maxVal, _, _ = cv2.minMaxLoc(myShiTomasi_dst)
    h = myShiTomasi_dst.shape[0]
    w = myShiTomasi_dst.shape[1]
#    print('print(np.shape(myShiTomasi_dst)))')
#    print(np.shape(myShiTomasi_dst))
    myShiTomasi_dst = np.reshape(myShiTomasi_dst, (h * w))
    myShiTomasi_dst[myShiTomasi_dst.argsort()[0:-maxCorners+1]] = myShiTomasi_minVal
    myShiTomasi_dst = np.reshape(myShiTomasi_dst, (h, w))
#    print(np.shape(myShiTomasi_dst))
    feature_points = []
    for i in range(src_gray.shape[0]):
        for j in range(src_gray.shape[1]):
            if myShiTomasi_dst[i,j] > myShiTomasi_minVal + ( myShiTomasi_maxVal - myShiTomasi_minVal )*qualityLevel/max_qualityLevel:
                
                feature_points.append([[i, j]])
                
    return feature_points
def draw_features(src, feature_points):
    import random as rng
    rng.seed(12345)
    for i, j in feature_points:
        cv2.circle(src, (j,i), 4, (rng.randint(0,256), rng.randint(0,256), rng.randint(0,256)), cv2.FILLED)
